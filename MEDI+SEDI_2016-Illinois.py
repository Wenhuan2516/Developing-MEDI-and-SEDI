...
df_env_sd = df_env_sd[df_env_sd['State Code'] == '17']
df_env_sd.head()
...
df_env= df_env[df_env['State Code'] == '17']
df_env.head()
...
weight = {'elevation': 0.16, 'mean_temp': 0.17, 'station_pressure': -0.18, 'visibility': 0.01, 'wind_speed': -0.16, 'precipitation': -0.21, 'Relative_Humidity': 0.27, 
          'Heat_Index': 0.14, 'fog': 0.09, 'rain': -0.12, 'snow': -0.34, 'hail': -0.12, 'thunder': 0.18, 'tornado': -0.20, 'PM2.5': -0.03, 'Ozone': -0.34, 'Humidity_Temp_Index': 0.19}
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='SuicideDeathRate',
                           ...
                           range_color=(0, 35),
                           scope="usa",
                           labels={'SuicideDeathRate':'SuicideRate_Illinois'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='elevation',
                           ...
                           range_color=(0, 300),
                           scope="usa",
                           labels={'elevation':'Elevation_Illinois'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='mean_temp',
                           ...
                           range_color=(20, 75),
                           scope="usa",
                           labels={'mean_temp':'Mean_Temp_Illinois'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='Relative_Humidity',
                           ...
                           range_color=(40, 80),
                           scope="usa",
                           labels={'Relative_Humidity':'Relative_Humidity_Illinois'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
