...
df_env_sd = df_env_sd[df_env_sd['State Code'] == '56']
df_env_sd.head()
...
df_env= df_env[df_env['State Code'] == '56']
df_env.head()
...
weight = {'elevation': 0.32, 'mean_temp': -0.23, 'station_pressure': -0.29, 'visibility': 0.03, 'precipitation': -0.28, 'Relative_Humidity': -0.08, 
          'Heat_Index': -0.28, 'fog': -0.22, 'rain': -0.24, 'snow': -0.18, 'hail': -0.03, 'thunder': -0.01, 'PM2.5': 0.23, 'Humidity_Temp_Index': -0.4}
...
variable = ['elevation', 'mean_temp', 'station_pressure', 'visibility','precipitation', 'Relative_Humidity', 'Heat_Index', 'fog',
            'rain', 'snow', 'hail', 'thunder','PM2.5', 'Humidity_Temp_Index']
for col in variable:
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='SuicideDeathRate',
                           ...
                           range_color=(0, 35),
                           scope="usa",
                           labels={'SuicideDeathRate':'SuicideRate_Wyoming'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='elevation',
                           ...
                           range_color=(0, 2500),
                           scope="usa",
                           labels={'elevation':'Elevation_Wyoming'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='mean_temp',
                           ...
                           range_color=(20, 60),
                           scope="usa",
                           labels={'mean_temp':'Mean_Temp_Wyoming'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='Relative_Humidity',
                           ...
                           range_color=(20, 70),
                           scope="usa",
                           labels={'Relative_Humidity':'Relative_Humidity_Wyoming'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
