...
df_env_sd = df_env_sd[df_env_sd['State Code'] == '20']
df_env_sd.head()
...
df_env= df_env[df_env['State Code'] == '20']
df_env.head()
...
weight = {'elevation': 0.03, 'mean_temp': 0.36, 'station_pressure': -0.14, 'visibility': 0.15, 'wind_speed': -0.13, 'precipitation': -0.25, 'Relative_Humidity': -0.01, 
          'Heat_Index': 0.34, 'fog': 0.16, 'rain': -0.09, 'snow': -0.27, 'hail': -0.25, 'tornado': -0.17, 'PM2.5': 0.21, 'Ozone': -0.03, 'Humidity_Temp_Index': 0.41}
...
variable = ['elevation', 'mean_temp', 'station_pressure', 'visibility', 'wind_speed', 'precipitation', 'Relative_Humidity', 'Heat_Index', 'fog',
            'rain', 'snow', 'hail', 'tornado', 'PM2.5', 'Ozone', 'Humidity_Temp_Index']
for col in variable:
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='SuicideDeathRate',
                           ...
                           range_color=(0, 35),
                           scope="usa",
                           labels={'SuicideDeathRate':'SuicideRate_Kansas'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='elevation',
                           ...
                           range_color=(0, 1000),
                           scope="usa",
                           labels={'elevation':'Elevation_Kansas'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='mean_temp',
                           ...
                           range_color=(0, 75),
                           scope="usa",
                           labels={'mean_temp':'Mean_Temp_Kansas'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
...
fig = px.choropleth(df_env_sd, geojson=counties, locations='fips', color='Relative_Humidity',
                           ...
                           range_color=(40, 80),
                           scope="usa",
                           labels={'Relative_Humidity':'Relative_Humidity_Kansas'}
                          )
fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
fig.show()
