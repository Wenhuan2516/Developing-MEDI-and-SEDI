...
ur_files = ddf.read_csv(r"Temperature/US_counties_weather_2015.csv", dtype={'eightieth_percentile_income': 'float64','fips': 'object','fog': 'float64','hail': 'float64',
...
df_svi_2015 = pd.read_csv(r'SVI_2014_US_COUNTY.csv', dtype = {'FIPS': str})
df_svi_2015
...
df_svi_2015 = df_svi_2015[['COUNTY', 'ST_ABBR', 'FIPS', 'EP_POV', 'EP_UNEMP', 'EP_PCI', 'EP_NOHSDP', 'EP_AGE65', 'EP_AGE17', 
...
df_svi_2015 = df_svi_2015.rename(columns = {'COUNTY': 'county', 'ST_ABBR': 'state', 'FIPS': 'fips', 'EP_POV': 'percent_below_poverty', 'EP_UNEMP': 'percent_unemployed_CDC', 'EP_PCI': 'per_capita_income', 
...
df_svi_2015['county'] = df_svi_2015['county'] + ', ' + df_svi_2015['state']
...
correlation_matrix(df_svi_2015)
...
df_temp_2015 = df_svi[['state', 'county', 'fips','station_id', 'station_name', 'lat', 'lon', 'date', 'mean_temp', 
...
days = df_temp_2015['date'].unique().tolist()
len(days)
...
df_temp_2015['dewpoint'].isnull().sum()
...
df_temp_county = df_temp_2015.groupby(['state', 'county', 'fips']).mean()
...
df_temp_2015['Relative_Humidity'] = df_temp_2015.apply(lambda x: RH_Calculator(x['mean_temp'], x['dewpoint']), axis=1)
...
df_temp_2015['Heat_Index'] = df_temp_2015.apply(lambda x: heat_index_calculation(x['mean_temp'], x['Relative_Humidity']), axis=1)
...
df_temp_2015.to_csv('daily_temp_2015.csv')
...
df_pm_2015 = df_pm[df_pm['Year'] == 2015]
df_pm_2015 = df_pm_2015.rename(columns = {'State': 'state', 'CountyFIPS': 'fips', 'County': 'county'})
df_pm_2015 = df_pm_2015.groupby(['state', 'fips', 'county']).mean()
df_pm_2015 = df_pm_2015.reset_index()
df_pm_2015
...
df_pm_2015 = df_pm_2015.rename(columns = {'Value': 'PM2.5'})
df_pm_2015
...
df_oz_2015 = df_oz[df_oz['Year'] == 2015]
df_oz_2015 = df_oz_2015.rename(columns = {'State': 'state', 'CountyFIPS': 'fips', 'County': 'county', 'Value': 'Ozone'})
df_oz_2015 = df_oz_2015.groupby(['state', 'fips', 'county']).mean()
df_oz_2015 = df_oz_2015.reset_index()
df_oz_2015
...
df_air = df_pm_2015.merge(df_oz_2015, on = ['state','fips', 'Year', 'StateFIPS', 'county'], how = 'left')
df_air.head()
...
df_sd_15 = pd.read_csv(r'SuicideRate_Imputed/SuicideRate_Imputed_2015.csv', dtype={"fips": str})
...
df_env_sd = df_env.merge(df_sd_15, on = ['fips'], how = 'left')
...
weight = {'mean_temp': -0.21, 'visibility': 0.06, 'wind_speed': 0.04, 'precipitation': -0.07, 'fog': -0.02, 'rain': -0.08, 'snow': 0.10, 'hail': -0.04,
          'thunder': 0.001, 'tornado': -0.02, 'PM2.5': -0.26, 'Ozone': -0.12}
...
df_SEDI = df_svi_2015.merge(df_sd_15, on = ['fips'], how = 'left')
...
df_SEDI.to_csv('Social_Economic_Suicide_2015.csv')


# In[83]:


weight = {'percent_below_poverty': 0.07, 'per_capita_income': 0.01, 'percent_unemployed_CDC': -0.14, 'percent_no_highschool_diploma': -0.06, 
         'percent_age_65_and_older': 0.16, 'percent_age_17_and_younger': -0.001, 'percent_disabled': 0.13, 'percent_single_parent_households_CDC': -0.03,
         'percent_minorities': -0.08, 'percent_limited_english_abilities': -0.16, 'percent_multi_unit_housing': -0.19,
        'percent_mobile_homes': 0.09, 'percent_overcrowding': 0.28, 'percent_no_vehicle': 0.15, 'percent_institutionalized_in_group_quarters': -0.01}
...
fig = px.choropleth(df_svi_2015, geojson=counties, locations='fips', color='SVI',
                           color_continuous_scale="rainbow",
                           range_color=(0, 1),
                           scope="usa",
                           hover_name="county",
...
df_final = df_svi_2015.merge(df_MEDI, on = ['fips'], how = 'left')
...
df_sd = pd.read_csv(r'SuicideRate_Imputed/SuicideRate_Imputed_2016.csv',dtype={"fips": str})
...
df_index_current = df_index5.merge(df_sd_15, on = ['county', 'fips'], how = 'left')
...
df_index_current['Year'] = '2015'


# In[148]:


df_index3.to_csv('All_Index_Next_Year_2015.csv')


# In[149]:


df_index_current.to_csv('All_Index_Current_Year_2015.csv')
